...
class DoCalc(ABC):
    ...
    def apply_independence(self, pr: Pr, target_var: Union[Variable, Set[Variable], List[Variable], Dict[Variable,int]] = None,
                           method: str = 'remove'):
        """
        Apply the independence condition to the expression.

        Parameters
        ----------
        expr : sp.Expr
            The expression to which the independence condition will be applied.
        do_vars : List[str]
            The list of variables involved in the do operation.

        Returns
        -------
        sp.Expr
            The modified expression after applying the independence condition.
        """
    ...
    def apply_consistency(self, pr: Pr, target_var: Union[Variable, Set[Variable], List[Variable]],
                          ...
                          method: str = 'remove'):
        """
        Apply the consistency condition to the expression.

        Parameters
        ----------
        expr : sp.Expr
            The expression to which the consistency condition will be applied.
        do_vars : List[str]
            The list of variables involved in the do operation.
        ...
        -------
        sp.Expr
            The modified expression after applying the consistency condition.
        """
    ...
    def apply_ctf_unnest(self, pr: Pr, target_var: Union[Variable, Set[Variable], List[Variable]] = None):
        """
        Apply the unnesting condition to the expression.

        Parameters
        ----------
        expr : sp.Expr
            The expression to which the unnesting condition will be applied.
        do_vars : List[str]
            The list of variables involved in the do operation.
        ...
        -------
        sp.Expr
            The modified expression after applying the unnesting condition.
        """
    ...
    def apply_exclusion(self, pr:Pr, target_var: Union[Variable,Set[Variable], List[Variable]],
        ...
        """
        Apply the exclusion condition to the target_var.

        Parameters
        ----------
        target_var : Union[Variable, Set[Variable], List[Variable]]
            The target_var or nodes to which the exclusion condition will be applied.
